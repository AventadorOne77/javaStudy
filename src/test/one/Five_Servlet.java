...
public class Five_Servlet {
    ...
    //36、Servlet API中forward()与redirect()的区别？
    /* 1.从地址栏显示来说
    ...
    //37、request.getAttribute()和 request.getParameter()有何区别?
    ...
    //38、jsp静态包含和动态包含的区别
    /* 1、<%@include file="xxx.jsp"%>为jsp中的编译指令，其文件的包含是发生在jsp向servlet转换的时期，而<jsp:include page="xxx.jsp">是jsp中的动作指令，
          其文件的包含是发生在编译时期，也就是将java文件编译为class文件的时期
       ...
       3、使用静态包含，包含页面和被包含页面的request对象为同一对象，因为静态包含只是将被包含的页面的内容复制到包含的页面中去；而动态包含包含页面和被包含页面不是同一个页面，
          被包含的页面的request对象可以取到的参数范围要相对大些，不仅可以取到传递到包含页面的参数，同样也能取得在包含页面向下传递的参数 */

    //39、MVC的各个部分都有那些技术来实现?如何实现?
    /* MVC是Model－View－Controller的简写。
    1.Model代表的是应用的业务逻辑（通过JavaBean，EJB组件实现），
    2.View是应用的表示面（由JSP页面产生），
    3.Controller是提供应用的处理过程控制（一般是一个Servlet），
    ...
    //41、什么是cookie？Session和cookie有什么区别？
    ...
    //43、tomcat容器是如何创建servlet类实例？用到了什么原理？
